...
public class ConfigContext
{
	...
	static String configFile = getSfipFourinoneConfigFilePath();
	...
	private static String getSfipFourinoneConfigFilePath() {
		// configFile = new
		// File("").getAbsolutePath()+File.separator+"config.xml";
		String basePath = System.getenv("SFIP_PARENT_HOME");
		...
		String currentConfigFile = basePath
				+ System.getProperty("file.separator")
				+ "sfip-fourinone-config.xml";
		File f = new File(currentConfigFile);
		if (!f.exists()) {
			basePath = System.getenv("SFIP_HOME");
			if (basePath != null || !"".equals(basePath)) {
				basePath = basePath + System.getProperty("file.separator")
						+ "config";
			} else {
			...
			currentConfigFile = basePath + System.getProperty("file.separator")
					+ "sfip-fourinone-config.xml";
			f = new File(currentConfigFile);
			if (!f.exists()) {
				currentConfigFile = new File("").getAbsolutePath()
						+ File.separator + "config.xml";
			}
		}
		System.out.println("current SfipFourinoneConfigFile="
				+ currentConfigFile);
